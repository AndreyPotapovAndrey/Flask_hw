...
engine = create_engine(PG_DSN)  # Такая фабрика подключений, которая создаёт много подключений внутри себя, управляет
# ими и выдаёт их для сессии
Session = sessionmaker(bind=engine)  # Создаём базовый класс для сессии. Указываем, чтобы подключение брал из engine
